...
const MetricCard = ({ icon: Icon, title, value, change, trend, color = 'blue', className = '' }) => {
  const { isDarkMode } = useTheme();
...
const DataVisualization = ({ data, type = 'overview', className = '' }) => {
  const { isDarkMode } = useTheme();
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      >
        ...
        <div className="space-y-3">
          {[
            { action: 'Content generated', type: 'Blog Post', time: '2 minutes ago', status: 'success' },
            { action: 'A/B test completed', type: 'Email Campaign', time: '15 minutes ago', status: 'info' },
            { action: 'Quality analysis', type: 'Social Media', time: '1 hour ago', status: 'warning' },
            { action: 'Template created', type: 'Marketing Copy', time: '2 hours ago', status: 'success' }
          ].map((activity, index) => (
